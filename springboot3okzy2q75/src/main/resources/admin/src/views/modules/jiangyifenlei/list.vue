			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 10px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"none","borderRadius":"10px","flexWrap":"wrap","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">讲义分类</label>
						<el-input v-model="searchForm.jiangyifenlei" placeholder="讲义分类" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"0px 0 10px -4px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jiangyifenlei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#4f7df5","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jiangyifenlei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0","boxShadow":"none","borderRadius":"0"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('jiangyifenlei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jiangyifenlei','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.jiangyifenlei!='' && this.searchForm.jiangyifenlei!=undefined){
					params['jiangyifenlei'] = '%' + this.searchForm.jiangyifenlei + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "jiangyifenlei/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
