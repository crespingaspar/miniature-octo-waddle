...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="讲义分类" prop="jiangyifenlei" >
					<el-input v-model="ruleForm.jiangyifenlei" placeholder="讲义分类" clearable  :readonly="ro.jiangyifenlei"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="讲义分类" prop="jiangyifenlei" >
					<el-input v-model="ruleForm.jiangyifenlei" placeholder="讲义分类" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jiangyifenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiangyifenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
