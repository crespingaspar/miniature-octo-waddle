				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','选课')" @click="xuankexinxiCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已选课')" type="success">
								...
								if (data && data.code === 0) {
									if(this.shBatchForm.sfsh!='待审核'){
										for(let x in this.shList){
											await this.$http.get('jiaoshi/query',{params: {
												gonghao: this.shList[x].gonghao
											}}).then(async rs=>{
												if(rs.data.data.email) {
													await this.$http.get('jiaoshi/sendemail/content',{params: {
														email: rs.data.data.email,
														title: '课程信息',
														content: `你提交的“课程信息”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
													}}).then(rs=>{})
												}
												let param = {
