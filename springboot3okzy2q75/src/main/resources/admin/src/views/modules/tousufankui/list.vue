		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 10px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"0px 0 10px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('tousufankui','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('tousufankui','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "tousufankui/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
											...
											await this.$http.get('xuesheng/query',{params: {
												...
												if(rs.data.data.email) {
													await this.$http.get('xuesheng/sendemail/content',{params: {
														...
														title: '投诉反馈',
														content: `你提交的“投诉反馈”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
													}}).then(rs=>{})
												}
												let param = {
