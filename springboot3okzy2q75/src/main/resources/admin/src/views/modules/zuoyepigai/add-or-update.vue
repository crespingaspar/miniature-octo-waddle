	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="作业名称" prop="zuoyemingcheng" >
				...
				<el-form-item v-else class="input" label="科目" prop="kemu" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="成绩" prop="chengji" >
					<el-input v-model.number="ruleForm.chengji" placeholder="成绩" clearable  :readonly="ro.chengji"></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.pigaishijian" label="批改时间" prop="pigaishijian" >
			...
			<el-form-item v-else-if="ruleForm.pingyu" label="评语" prop="pingyu"  class="textBox">
				<span class="text">{{ruleForm.pingyu}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'" class="editorBox" label="批改状况" prop="pigaizhuangkuang" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.pigaizhuangkuang" 
					class="editor"
					myQuillEditor="pigaizhuangkuang"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.pigaizhuangkuang" label="批改状况" prop="pigaizhuangkuang"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.pigaizhuangkuang"></span>
			</el-form-item>
		...
		data() {
			...
			return {
				...
				rules: {
					...
					pigaizhuangkuang: [
						{ required: true, message: '批改状况不能为空', trigger: 'blur' },
					],
					pigaishijian: [
					],
					gonghao: [
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.pigaizhuangkuang = this.ruleForm.pigaizhuangkuang.replace(reg,'../../../springboot3okzy2q75/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
