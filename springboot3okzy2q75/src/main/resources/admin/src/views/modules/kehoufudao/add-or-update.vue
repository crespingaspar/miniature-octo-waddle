	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="辅导时间" prop="fudaoshijian" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.fudaoshijian" 
						type="datetime"
						:readonly="ro.fudaoshijian"
						:picker-options="fudaoshijianPickerOptions"
						placeholder="辅导时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.fudaoshijian" label="辅导时间" prop="fudaoshijian" >
					<el-input v-model="ruleForm.fudaoshijian" placeholder="辅导时间" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="学习效果" prop="xuexixiaoguo" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="学习效果"
					v-model="ruleForm.xuexixiaoguo" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.xuexixiaoguo" label="学习效果" prop="xuexixiaoguo"  class="textBox">
				<span class="text">{{ruleForm.xuexixiaoguo}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'" class="editorBox" label="辅导内容" prop="fudaoneirong" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.fudaoneirong" 
					class="editor"
					myQuillEditor="fudaoneirong"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.fudaoneirong" label="辅导内容" prop="fudaoneirong"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.fudaoneirong"></span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			this.ruleForm.fudaoshijian = this.getCurDateTime()
		},
		methods: {
			imgPreView(url){
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.fudaoneirong = this.ruleForm.fudaoneirong.replace(reg,'../../../springboot3okzy2q75/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `kehoufudao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.kehoufudaoCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
