...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="讲义名称" prop="jiangyimingcheng" >
					<el-input v-model="ruleForm.jiangyimingcheng" placeholder="讲义名称" clearable  :readonly="ro.jiangyimingcheng"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="讲义名称" prop="jiangyimingcheng" >
					<el-input v-model="ruleForm.jiangyimingcheng" placeholder="讲义名称" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="讲义分类" prop="jiangyifenlei" >
					<el-select :disabled="ro.jiangyifenlei" v-model="ruleForm.jiangyifenlei" placeholder="请选择讲义分类"  filterable>
						<el-option
							v-for="(item,index) in jiangyifenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="讲义分类" prop="jiangyifenlei" >
					<el-input v-model="ruleForm.jiangyifenlei"
						placeholder="讲义分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="fileupload" v-if="type!='info'&& !ro.fujianxiazai" label="附件下载" prop="fujianxiazai" >
					<file-upload
						tip="点击上传附件下载"
						action="file/upload"
						:limit="1"
						:type="2"
						:multiple="true"
						:disabled="ro.fujianxiazai"
						:fileUrls="ruleForm.fujianxiazai?ruleForm.fujianxiazai:''"
						@change="fujianxiazaiUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item v-else-if="ruleForm.fujianxiazai" label="附件下载" prop="fujianxiazai" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.fujianxiazai)">
			...
			<el-form-item class="textarea" v-if="type!='info'" label="考点笔记" prop="kaodianbiji" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="考点笔记"
					v-model="ruleForm.kaodianbiji" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.kaodianbiji" label="考点笔记" prop="kaodianbiji"  class="textBox">
				<span class="text">{{ruleForm.kaodianbiji}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'" class="editorBox" label="讲义内容" prop="jiangyineirong" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.jiangyineirong" 
					class="editor"
					myQuillEditor="jiangyineirong"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.jiangyineirong" label="讲义内容" prop="jiangyineirong"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.jiangyineirong"></span>
			</el-form-item>
	...
	export default {
		data() {
			...
			return {
				...
				ro:{
					jiangyimingcheng : false,
					jiangyifenlei : false,
					tupian : false,
					fujianxiazai : false,
					kaodianbiji : false,
					jiangyineirong : false,
					gonghao : false,
				...
				jiangyifenleiOptions: [],

				rules: {
					jiangyimingcheng: [
						{ required: true, message: '讲义名称不能为空', trigger: 'blur' },
					],
					jiangyifenlei: [
						{ required: true, message: '讲义分类不能为空', trigger: 'blur' },
					],
					tupian: [
					],
					fujianxiazai: [
						{ required: true, message: '附件下载不能为空', trigger: 'blur' },
					],
					kaodianbiji: [
						{ required: true, message: '考点笔记不能为空', trigger: 'blur' },
					],
					jiangyineirong: [
						{ required: true, message: '讲义内容不能为空', trigger: 'blur' },
					],
					gonghao: [
					],
					jiaoshixingming: [
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						if(o=='jiangyimingcheng'){
							this.ruleForm.jiangyimingcheng = obj[o];
							this.ro.jiangyimingcheng = true;
							continue;
						}
						if(o=='jiangyifenlei'){
							this.ruleForm.jiangyifenlei = obj[o];
							this.ro.jiangyifenlei = true;
							continue;
						}
						if(o=='tupian'){
						...
						if(o=='kaodianbiji'){
							this.ruleForm.kaodianbiji = obj[o];
							this.ro.kaodianbiji = true;
							continue;
						}
						if(o=='jiangyineirong'){
							this.ruleForm.jiangyineirong = obj[o];
							this.ro.jiangyineirong = true;
							continue;
						}
						if(o=='gonghao'){
				...
				this.$http({
					url: `option/jiangyifenlei/jiangyifenlei`,
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.jiangyifenleiOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jiangyineirong = this.ruleForm.jiangyineirong.replace(reg,'../../../springboot3okzy2q75/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `dianzijiangyi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.dianzijiangyiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
