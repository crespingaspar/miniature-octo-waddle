...
<template>
	<div class="main-content" :style='{"padding":"30px"}'>
			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 10px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"none","borderRadius":"10px","flexWrap":"wrap","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">课程号</label>
						<el-input v-model="searchForm.kechenghao" placeholder="课程号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"0px 0 10px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('xuankexinxi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe9" :style='{"margin":"0 0px","fontSize":"16px","color":"#333","height":"auto"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('xuankexinxi','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('xuankexinxi','学习')" @click="xuexijinduCrossAddOrUpdateHandler(scope.row,'cross','是','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","height":"40px"}'></span>
								学习
							</el-button>
							<el-button class="btn8" v-if="isAuth('xuankexinxi','退费')" @click="tuifeishenqingCrossAddOrUpdateHandler(scope.row,'cross','是','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","height":"40px"}'></span>
								退费
							</el-button>
							<el-button class="edit" v-if=" isAuth('xuankexinxi','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
		...
		<tuifeishenqing-cross-add-or-update v-if="tuifeishenqingCrossAddOrUpdateFlag" :parent="this" ref="tuifeishenqingCrossaddOrUpdate"></tuifeishenqing-cross-add-or-update>
...
<script>
	...
	import tuifeishenqingCrossAddOrUpdate from "../tuifeishenqing/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				tuifeishenqingCrossAddOrUpdateFlag: false,
		...
		components: {
			...
			tuifeishenqingCrossAddOrUpdate,
		},
		methods: {
			queryChange(arr){
				for(let x in arr) {
			...
			tuifeishenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "warning",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.tuifeishenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','xuankexinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tuifeishenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tuifeishenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				if(row.sfsh!='是'){
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "xuankexinxi/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
											...
											await this.$http.get('xuesheng/query',{params: {
												...
												if(rs.data.data.email) {
													await this.$http.get('xuesheng/sendemail/content',{params: {
														...
														title: '选课信息',
														content: `你提交的“选课信息”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
													}}).then(rs=>{})
												}
												let param = {
