			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 10px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"none","borderRadius":"10px","flexWrap":"wrap","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">讲义名称</label>
						<el-input v-model="searchForm.jiangyimingcheng" placeholder="讲义名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","margin":"0px 0 10px -4px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('dianzijiangyi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#4f7df5","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('dianzijiangyi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0","boxShadow":"none","borderRadius":"0"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
												prop="jiangyimingcheng"
						label="讲义名称">
						<template slot-scope="scope">
							{{scope.row.jiangyimingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="jiangyifenlei"
					...
					<el-table-column :resizable='true' prop="fujianxiazai" label="附件下载">
						<template slot-scope="scope">
							<el-button v-if="scope.row.fujianxiazai" type="text" size="small" @click="download(scope.row.fujianxiazai )">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('dianzijiangyi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan13" :style='{"margin":"0 0px","fontSize":"14px","color":"#4f7df5","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('dianzijiangyi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai17" :style='{"margin":"0 0px","fontSize":"14px","color":"#38acde","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('dianzijiangyi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan13" :style='{"margin":"0 0px","fontSize":"14px","color":"#4f7df5","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('dianzijiangyi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.jiangyimingcheng!='' && this.searchForm.jiangyimingcheng!=undefined){
					params['jiangyimingcheng'] = '%' + this.searchForm.jiangyimingcheng + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "dianzijiangyi/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdianzijiangyi',query:{refid:id}});
			},
			// 下载
