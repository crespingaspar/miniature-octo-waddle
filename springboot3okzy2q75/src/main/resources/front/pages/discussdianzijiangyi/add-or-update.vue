	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussdianzijiangyi`, this.ruleForm.id);
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`discussdianzijiangyi`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`discussdianzijiangyi`, this.ruleForm);
				}else{
					oet = await this.$api.add(`discussdianzijiangyi`, this.ruleForm);
				}
				uni.setStorageSync('discussdianzijiangyiCleanType',true);
				if(this.cross){
