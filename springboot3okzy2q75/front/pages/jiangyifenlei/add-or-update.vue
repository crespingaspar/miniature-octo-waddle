	...
	<view :style='{"width":"100%","padding":"24rpx","position":"relative","background":"#fff","height":"100%"}' v-if="showType==1">
		<form :style='{"width":"100%","padding":"48rpx 0 48rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"4rpx 40rpx","margin":"0 0 40rpx 0","borderColor":"#d6b8ff","alignItems":"center","borderRadius":"60rpx","borderWidth":"2rpx","background":"#fff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"padding":"0 20rpx 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","width":"auto","lineHeight":"80rpx","fontSize":"30rpx","fontWeight":"500"}' class="title">讲义分类</view>
				<input :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"30rpx","height":"80rpx"}' :disabled="ro.jiangyifenlei" v-model="ruleForm.jiangyifenlei" placeholder="讲义分类"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jiangyifenlei`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap(subMitType=null) {
				...
				//跨表计算判断
				var obj;
				if((!this.ruleForm.jiangyifenlei)){
					this.$utils.msg(`讲义分类不能为空`);
					return
				}
				//更新跨表属性
				var crossuserid;
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`jiangyifenlei`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`jiangyifenlei`, this.ruleForm);
				}else{
					oet = await this.$api.add(`jiangyifenlei`, this.ruleForm);
				}
				if(this.cross){
