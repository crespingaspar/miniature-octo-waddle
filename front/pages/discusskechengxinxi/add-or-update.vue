		...
		methods: {
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.ruleForm.id){
					...
					oet = await this.$api.add(`discusskechengxinxi`, this.ruleForm);
				}
				uni.setStorageSync('discusskechengxinxiCleanType',true);
				let rs2 = await this.$api.list('discusskechengxinxi',{
					page: 1,
					limit: 10000,
				...
				let rs3 = await this.$api.info('kechengxinxi',this.ruleForm.refid)
				rs3.data.totalscore = rs2.data.list.length?Number((totalscore / rs2.data.list.length).toFixed(2)):0
				await this.$api.update('kechengxinxi',rs3.data)
				if(this.cross){
